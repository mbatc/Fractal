...
namespace Fractal
{
  ...
  bool GLWindowRenderTarget::SetFormat(flIN const RenderTargetOptions* pOptions)
  {
    ...
    {
    case PixelComponentType_Float16:
      componentWidth = 16;
      floatPixel = true;
      break;
    case PixelComponentType_Float32:
      componentWidth = 32;
      floatPixel = true;
      break;
    case PixelComponentType_UInt8:   componentWidth = 8;  break;
    case PixelComponentType_UInt16:  componentWidth = 16; break;
    case PixelComponentType_UInt32:  componentWidth = 32; break;
    case PixelComponentType_Int8:    componentWidth = 8;  break;
    case PixelComponentType_Int16:   componentWidth = 16; break;
    case PixelComponentType_Int32:   componentWidth = 32; break;
    case PixelComponentType_UNorm8:  componentWidth = 8;  break;
    case PixelComponentType_UNorm16: componentWidth = 16; break;
    ...
    {
    // Depth-only formats
    case DepthFormat_Float16: numDepthBits = 16; break;
    case DepthFormat_Float24: numDepthBits = 24; break;
    case DepthFormat_Float32: numDepthBits = 32; break;

    // Stencil formats
    case DepthFormat_Float24Stencil8:
      numDepthBits = 24;
      numStencilBits = 8;
      break;
    case DepthFormat_Float32Stencil8:
      numDepthBits = 32;
      numStencilBits = 8;
      break;
    }
    ...
    if (floatPixel)
      _AddAttribute(&attribList, WGL_PIXEL_TYPE_ARB, WGL_TYPE_RGBA_FLOAT_ARB);
    else
      _AddAttribute(&attribList, WGL_PIXEL_TYPE_ARB, WGL_TYPE_RGBA_ARB);

    // Depth Buffer Format
    if (numDepthBits > 0)
      _AddAttribute(&attribList, WGL_DEPTH_BITS_ARB, (int)numDepthBits);
    if (numStencilBits > 0)
      _AddAttribute(&attribList, WGL_STENCIL_BITS_ARB, (int)numStencilBits);

    // Other settings
    if (pOptions->sampleCount > 1)
      _AddAttribute(&attribList, WGL_SAMPLES_ARB, (int)pOptions->sampleCount);
    if (pOptions->stereoBuffer)
      _AddAttribute(&attribList, WGL_STEREO_ARB, GL_TRUE);
  ...
  Window* GLWindowRenderTarget::GetWindow() const
  {
    return m_pWindow;
  }
